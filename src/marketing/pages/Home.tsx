    ...
    <section className="pt-10 pb-16 grid md:grid-cols-[1.1fr_1fr] gap-10 items-center">
      ...
      <div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {/* RALLY BUTTON - External Redirect */}
          <a
...
const WhyOfflineSection: React.FC = () => {
  const items = [
    { title: "Always-on reliability", body: "Scenes, schedules and automations keep running even when Wi-Fi or ISP services fail." },
    { title: "Data stays at home", body: "M-OS processes data locally on the hub. You decide what, if anything, touches the cloud." },
    { title: "Instant response", body: "No cloud round-trip. Switches, sensors and scenes respond in milliseconds." },
    { title: "No subscription trap", body: "Core automation doesn’t depend on monthly cloud fees or external servers." },
  ];
      ...
      <div className="mt-6 grid sm:grid-cols-2 gap-4">
        {items.map((item) => (
          <div key={item.title} className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="text-xs text-[var(--text-muted,#9ca3af)]">{item.body}</div>
          </div>
        ))}
      </div>
    </section>
...
const HowItWorksSection: React.FC = () => {
  const steps = [
    { step: "Step 1", title: "Install the M-OS hub", body: "A compact edge controller installed inside your home." },
    { step: "Step 2", title: "Connect devices locally", body: "Retrofit modules and sensors talk directly to the hub over secure local protocols." },
    { step: "Step 3", title: "Configure scenes", body: "Use the Project-M app to define automations. All logic lives and runs locally." },
    { step: "Step 4", title: "Add optional cloud", body: "Remote access and notifications are optional layers on top of a functional offline core." },
  ];
  ...
  return (
    <section className="pb-14">
      ...
      <div className="mt-6 grid md:grid-cols-4 sm:grid-cols-2 gap-4">
        {steps.map((s) => (
          <div key={s.title} className="rounded-2xl border border-white/10 bg-black/40 p-4 flex flex-col gap-1">
            <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--mos-red,#ff3131)]">{s.step}</div>
            <div className="text-sm font-medium">{s.title}</div>
            <div className="text-xs text-[var(--text-muted,#9ca3af)]">{s.body}</div>
          </div>
        ))}
      </div>
    </section>
...
const SolutionsSection: React.FC = () => {
  const solutions = [
    { title: "Apartments", body: "Retrofit automation with minimal civil work. Ideal for 2/3 BHK homes." },
    { title: "Villas & farmhouses", body: "Lighting, climate, perimeter security and pumps — even with unstable networks." },
    { title: "Hotels & stays", body: "Guest experiences that stay consistent during cloud or ISP outages." },
    { title: "Offices & studios", body: "Access control, schedules, scenes and energy efficiency in shared spaces." },
  ];
    ...
    <section className="pb-14">
      ...
      <div className="mt-6 overflow-x-auto text-xs">
        <table className="min-w-[560px] border-separate border-spacing-y-2">
          ...
          <tbody>
            <CompareRow label="Works when internet is down" projectM="✔ Core automations continue" cloud="✖ Functions stop" />
            <CompareRow label="Where decisions run" projectM="✔ On your M-OS hub" cloud="✖ On remote servers" />
            <CompareRow label="Data location" projectM="✔ Inside your home" cloud="✖ Stored remotely" />
            <CompareRow label="Latency" projectM="✔ Milliseconds" cloud="✖ Internet dependent" />
          </tbody>
        </table>
      </div>
    </section>
...
const DeviceEcosystemSection: React.FC = () => {
  const devices = [
    { title: "M-OS hub", body: "The local brain that runs Project-M automation." },
    { title: "Retrofit modules", body: "Upgrade existing switchboards into smart controls." },
    { title: "Touch panels", body: "Premium glass or matte touch panels with shortcuts." },
    { title: "Sensors", body: "Motion, door, temperature, and leak detection." },
    { title: "Curtain controllers", body: "Automated openings based on time and light." },
    { title: "Energy modules", body: "Monitor and orchestrate heavy loads and motors." },
  ];
  ...
  return (
    <section className="pb-14">
      ...
      <div className="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        {devices.map((d) => (
          <div key={d.title} className="rounded-2xl border border-white/10 bg-black/40 p-4">
            <div className="text-sm font-medium mb-1">{d.title}</div>
            <div className="text-xs text-[var(--text-muted,#9ca3af)]">{d.body}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="pb-4">
      <div className="rounded-3xl border border-[var(--mos-red,#ff3131)]/40 bg-[linear-gradient(135deg,_rgba(255,49,49,0.16),_rgba(0,0,0,0.8))] p-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
        <div>
          <div className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted,#9ca3af)]">Get started</div>
          <div className="text-lg sm:text-xl font-semibold mt-1">Make your home the first offline smart home.</div>
          <div className="text-xs text-[var(--text-muted,#9ca3af)] mt-2 max-w-md">Share your floor plan and we'll map a plan tailored to your space.</div>
        </div>
...
const SectionHeader: React.FC<{ eyebrow: string; title: string; description?: string }> = ({ eyebrow, title, description }) => (
  <div className="max-w-2xl">
    <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--mos-red,#ff3131)] mb-1">{eyebrow}</div>
    <h2 className="text-xl sm:text-2xl font-semibold tracking-tight">{title}</h2>
    {description && <p className="mt-2 text-xs sm:text-sm text-[var(--text-muted,#9ca3af)]">{description}</p>}
  </div>
);

const PillCard: React.FC<{ title: string; value: string }> = ({ title, value }) => (
  <div className="rounded-xl bg-white/5 border border-white/10 px-3 py-2 flex flex-col gap-0.5">
    <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--text-muted,#9ca3af)]">{title}</div>
    <div className="text-xs font-medium">{value}</div>
  </div>
);

const ArchRow: React.FC<{ left: string; right: string }> = ({ left, right }) => (
  <div className="flex items-center justify-between gap-2">
    <span className="text-[10px] text-[var(--text-muted,#9ca3af)]">{left}</span>
    <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 border border-white/10">{right}</span>
  </div>
);

const CompareRow: React.FC<{ label: string; projectM: string; cloud: string }> = ({ label, projectM, cloud }) => (
  <tr>
    <td className="align-top pr-4 py-1 text-[var(--text-muted,#9ca3af)]">{label}</td>
    <td className="align-top pr-4 py-1"><span className="inline-flex px-2 py-1 rounded-full bg-black/40 border border-emerald-400/40 text-[10px]">{projectM}</span></td>
    <td className="align-top py-1"><span className="inline-flex px-2 py-1 rounded-full bg-black/30 border border-red-400/30 text-[10px]">{cloud}</span></td>
  </tr>
);
